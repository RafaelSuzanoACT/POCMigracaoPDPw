...
import {
  ModalidadeOperativaData,
  ModalidadeOperativaForm,
  ModalidadeOperativaIntervalo,
} from '../../../types/operatingMode';
 ...
 * - Coleta de modalidade operativa por intervalo de 30 min (48 intervalos)
 * - Visualização de dados em tabela dinâmica
 * - Suporte para edição individual ou todas as usinas
 */
const OperatingMode: React.FC = () => {
  // Estados
  const [formData, setFormData] = useState<ModalidadeOperativaForm>({
    dataPdp: '',
    codEmpresa: '',
    codUsina: '',
  ...
  const [dadosModalidade, setDadosModalidade] = useState<ModalidadeOperativaData[]>([]);
  ...
  // Carregar usinas térmicas quando empresa é selecionada
  useEffect(() => {
    ...
    const carregarUsinas = async () => {
      try {
        setLoading(true);
        // Mock de usinas térmicas - substituir por chamada real à API
        const usinasMock = ['UTE001', 'UTE002', 'UTE003', 'UTE004'];
        setUsinas(usinasMock);

        // Carregar dados de modalidade existentes
        await carregarDadosModalidade();
      } catch (error) {
  ...
  // Carregar dados de modalidade operativa da empresa/usina
  const carregarDadosModalidade = async () => {
    try {
      setLoading(true);
      // Mock de dados - substituir por chamada real à API
      const dadosMock: ModalidadeOperativaData[] = [];
      usinas.forEach((usina) => {
        for (let i = 1; i <= 48; i++) {
          dadosMock.push({
            codusina: usina,
            intmot: i,
            valmottran: Math.floor(Math.random() * 100),
            datpdp: formData.dataPdp,
          });
        }
      });
      setDadosModalidade(dadosMock);
    } catch (error) {
      setMessage({ text: 'Erro ao carregar dados de modalidade operativa', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUsinaChange = (codUsina: string) => {
    ...
    if (codUsina === 'TODAS') {
      ...
      const valoresUsina = dadosModalidade
        .filter((d) => d.codusina === codUsina)
        .sort((a, b) => a.intmot - b.intmot)
        .map((d) => d.valmottran);
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      const intervalos: ModalidadeOperativaIntervalo[] = formData.valores.map((valor, index) => ({
        intmot: index + 1,
        valmottran: valor,
      }));

      // Mock de salvamento - substituir por chamada real à API
      // const response = await api.post('/api/modalidade-operativa', {
  ...
  const usinasUnicas = useMemo(() => {
    const codigos = new Set(dadosModalidade.map((d) => d.codusina));
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div className={styles.title}>
          <img src="/images/tit_ModOpTermica.gif" alt="Modalidade Operativa Térmica" />
        </div>
      </div>
      ...
      <form className={styles.form} data-testid="operating-mode-form">
      ...
      <div className={styles.tableContainer}>
        <table className={styles.operatingModeTable} data-testid="operating-mode-table">
          <thead>
            <tr>
          ...
          <tbody>
            {intervalos.map((intervalo, index) => (
              <tr key={index}>
                ...
                {usinasUnicas.map((usina) => {
                  const dado = dadosModalidade.find(
                    (d) => d.codusina === usina && d.intmot === index + 1
                  );
                  return (
                    <td
                      key={usina}
